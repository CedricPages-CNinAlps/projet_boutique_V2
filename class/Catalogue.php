...
class Catalogue
{
    ...
    public function __construct()
    {
        ...
        foreach ($article->fetchAll(PDO::FETCH_ASSOC) as $data) {
            ...
            echo '</pre>';*/
            if ($data['color']){
                $new = new ArticleSpecific($data['id'], $data['name'], $data['description'], $data['price'], $data['weight'], $data['quantity'], $data['image'], $data['availability'], $data['categorie_id'], $data['color']);
            } else {
